...
public class CassandraCQLClient extends DB {
  ...
  private Map<String,Integer> readAccesses = new HashMap<>();
  private Map<String,Integer> writeAccesses = new HashMap<>();
  private int reads = 0;
  private int inserts = 0;
  private int updates = 0;
  private int deletes = 0;
  private Map<Integer,Set<String>> groups = new HashMap<>(10000);

  /**
   * Initialize any state for this DB. Called once per DB instance; there is one
   * DB instance per client thread.
   * DO NOTHING FOR GETTING WORKLOAD ONLY
   */
  @Override
  ...
  public void cleanup() throws DBException {
    ...
    try {
      readAccesses = sortByValue((HashMap<String, Integer>) readAccesses);
      writeAccesses = sortByValue((HashMap<String, Integer>) writeAccesses);

      long threadId = Thread.currentThread().getId();

      BufferedWriter writer = new BufferedWriter(new FileWriter("/Users/enrico/Desktop/HybridDB/stats/reads" + threadId + ".txt",false));
      for (Map.Entry<String, Integer> entry : readAccesses.entrySet()) {
        String id = entry.getKey();
        Integer amount = entry.getValue();
        writer.write(id + " " + amount + '\n');
      }
      writer.close();
      writer = new BufferedWriter(new FileWriter("/Users/enrico/Desktop/HybridDB/stats/writes" + threadId + ".txt",false));
      for (Map.Entry<String, Integer> entry : writeAccesses.entrySet()) {
        String id = entry.getKey();
        Integer amount = entry.getValue();
        writer.write(id + " " + amount + '\n');
      }
      ...
      System.out.println("Number of items with stats: " + writeAccesses.size());
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    try {
      ...
      for (ColumnDefinitions.Definition def : cd) {
        ...
        readAccesses.put(key,readAccesses.getOrDefault(key,0)+1);
        reads++;
    ...
    try {
      ...
      while (!rs.isExhausted()) {
        ...
        for (int i = 0 ; i < recordcount ; i++) {
          int index = Integer.parseInt(startkey) + i;
          readAccesses.put(Integer.toString(index), readAccesses.getOrDefault(Integer.toString(index), 0) + 1);
        }
  ...
  public Status update(String table, String key, Map<String, ByteIterator> values) {
    ...
    try {
      ...
      session.execute(boundStmt);
        writeAccesses.put(key, writeAccesses.getOrDefault(key, 0) + 1);
        updates++;
      return Status.OK;
    } catch (Exception e) {
